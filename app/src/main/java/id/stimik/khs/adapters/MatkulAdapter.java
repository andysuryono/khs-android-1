...
public class MatkulAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    List<ItemMatKul> listItem;
    ...
    public MatkulAdapter(Context ctx) {
        this.ctx = ctx;
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof OriginalViewHolder) {
            ...
            ItemMatKul item = listItem.get(position);
            view.tvNama.setText(item.getNama());
            view.tvNim.setText(item.getKode());
            view.tvNamaDosen.setText(item.getDosen().getNama());
    ...
    public void add(ItemMatKul item) {
        listItem.add(item);
        notifyItemInserted(listItem.size() + 1);
    }

    public void addAll(List<ItemMatKul> listItem) {
        for (ItemMatKul item : listItem) {
            add(item);
        }
    }
    ...
    public void swap(List<ItemMatKul> datas) {
    ...
    public ItemMatKul getItem(int pos) {
    ...
    public void setFilter(List<ItemMatKul> list) {
        listItem = new ArrayList<>();
        listItem.addAll(list);
        notifyDataSetChanged();
    }

    public List<ItemMatKul> getListItem() {
        return listItem;
    }
}
