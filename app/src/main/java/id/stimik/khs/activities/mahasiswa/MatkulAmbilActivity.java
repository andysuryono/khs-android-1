...
import id.stimik.khs.adapters.StudyAdapter;
...
public class MatkulAmbilActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_mahasiswa_ambil_matkul);
    ...
    private void getData() {
        ...
        etSemester.setText(id_semester+"");
    }
        ...
        if (response instanceof StudyResponse) {
            if (((StudyResponse) response).isSuccess()) {
                if (((StudyResponse) response).getData() != null && ((StudyResponse) response).getData().size() != 0) {
                    ...
                    etJumlahSks.setText(0 + " SKS");
                    etSisaSks.setText(0 + " SKS");
                    recyclerView.setAdapter(emptyAdapter);
                }
            } else {
                etJumlahSks.setText(0 + " SKS");
                etSisaSks.setText(0 + " SKS");
                recyclerView.setAdapter(emptyAdapter);
                CommonUtil.showToast(MatkulAmbilActivity.this, ((StudyResponse) response).getMessage());
            }
        ...
        AndroidNetworking.delete(NILAI + "/{id_studies}")
                ...
                .getAsJSONObject(new JSONObjectRequestListener() {
                    ...
                    public void onResponse(JSONObject response) {
                        refresh.setRefreshing(false);
                        CommonUtil.showToast(MatkulAmbilActivity.this, "Berhasil Di hapus");
                        loadData();
                    }
    ...
    void openAddDialog() {
        DialogAmbilMatkul dialogMahasiswa = DialogAmbilMatkul.newInstance(id_semester);
        FragmentManager manager = getSupportFragmentManager();
        FragmentTransaction transaction = manager.beginTransaction();
        transaction.add(android.R.id.content, dialogMahasiswa).addToBackStack(null).commit();
        dialogMahasiswa.setDialogSucces(new DialogAmbilMatkul.DialogSucces() {
            @Override
